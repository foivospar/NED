...
def define_G(input_nc, output_nc, prev_output_nc, ngf, n_downsampling, norm, gpu_ids=[], opt=[]):
    ...
    netG = Generator(input_nc, output_nc, prev_output_nc, ngf, n_downsampling, opt.n_blocks, norm_layer, opt.no_prev_output, opt.n_frames_G)
    if len(gpu_ids) > 0:
...
def define_D(input_nc, ndf, n_layers_D, norm='instance', num_D=1, getIntermFeat=False, is_image_discriminator=False, gpu_ids=[], opt=[]):
    norm_layer = get_norm_layer(norm_type=norm)
    netD = MultiscaleDiscriminator(input_nc, ndf, n_layers_D, norm_layer, num_D, getIntermFeat, opt.input_nc, opt.output_nc)
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
        netD.cuda(gpu_ids[0])
...
class Generator(nn.Module):
    def __init__(self, input_nc, output_nc, prev_output_nc, ngf, n_downsampling, n_blocks,
                 norm_layer, no_prev_output, n_frames_G, padding_type='reflect'):
        assert(n_blocks >= 0)
    ...
    def forward(self, input, img_prev):
        ...
        input = torch.cat([ch[:,:-1,:,:] for ch in torch.chunk(input, chunks=self.n_frames_G, dim=1)],1)
...
class MultiscaleDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
                 num_D=3, getIntermFeat=False, or_input_nc=6, or_output_nc=3):
        ...
        self.input_nc = or_input_nc
        self.output_nc = or_output_nc
    ...
    def forward(self, input):
        input = torch.cat((input[:,:self.input_nc,:,:], input[:,-self.output_nc:,:,:]), 1)
        num_D = self.num_D
        result = []
